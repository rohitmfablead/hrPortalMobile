...
fs.readdirSync(directoryPath).forEach(file => {
  ...
  const regex = /<View style={\[?styles\.formGroup[^>]*>[\s\S]*?<Text style={styles\.label}>(.*?)<\/Text>[\s\S]*?<View style={[^>]*inputContainer[^>]*}>[\s\S]*?<([A-Z][A-Za-z0-9]*)[^>]*\/>[\s\S]*?<TextInput([\s\S]*?)\/>[\s\S]*?<\/View>[\s\S]*?<\/View>/g;
  ...
  const regexMulti = /<View style={\[?styles\.formGroup[^>]*>[\s\S]*?<Text style={styles\.label}>(.*?)<\/Text>[\s\S]*?<View style={\[styles\.inputContainer[^>]*}>[\s\S]*?<([A-Z][A-Za-z0-9]*)[^>]*\/>[\s\S]*?<TextInput([\s\S]*?)\/>[\s\S]*?<\/View>[\s\S]*?<\/View>/g;
