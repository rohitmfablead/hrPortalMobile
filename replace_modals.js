...
fs.readdirSync(directoryPath).forEach(file => {
  ...
  if (content.includes("import CustomTextInput")) return;
  ...
  // 2. Replace formRow elements (with flex)
  // This is extremely hard with regex. Let's do a simpler regex that matches the whole <View> block.
  const regex = /<View style={\[?styles\.formGroup[^>]*>[\s\S]*?<Text style={styles\.label}>(.*?)<\/Text>[\s\S]*?<View style={[^>]*inputContainer[^>]*}>[\s\S]*?<([A-Z][A-Za-z0-9]*) color="#F97316" size={20} style={styles\.inputIcon} \/>[\s\S]*?<TextInput ([^>]*?)\/>[\s\S]*?<\/View>[\s\S]*?<\/View>/g;
  
  content = content.replace(regex, (match, label, IconName, textInputProps) => {
      // Extract flex wrapper if it exists
  ...
  // Multiline inputs
  const regexMulti = /<View style={\[?styles\.formGroup[^>]*>[\s\S]*?<Text style={styles\.label}>(.*?)<\/Text>[\s\S]*?<View style={\[styles\.inputContainer[^>]*}>[\s\S]*?<([A-Z][A-Za-z0-9]*) color="#F97316" size={20} style={styles\.inputIcon} \/>[\s\S]*?<TextInput ([^>]*?)\/>[\s\S]*?<\/View>[\s\S]*?<\/View>/g;
