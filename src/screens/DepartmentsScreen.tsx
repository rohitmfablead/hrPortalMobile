...
import { fetchDepartments } from '../redux/slices/masterSlice';
import * as masterService from '../services/masterService';

export default function DepartmentsScreen() {
  const dispatch = useDispatch<AppDispatch>();
  const { departments, loading } = useSelector((state: RootState) => state.master);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await masterService.updateDepartment(editingId, { name, description });
      } else {
        await masterService.createDepartment({ name, description });
      }
      hideModal();
      dispatch(fetchDepartments());
    } catch (error: any) {
  ...
  const handleDelete = (id: string) => {
    Alert.alert('Confirm Delete', 'Are you sure you want to delete this department?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            await masterService.deleteDepartment(id);
            dispatch(fetchDepartments());
          } catch (error: any) {
