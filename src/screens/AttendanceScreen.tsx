...
export default function AttendanceScreen() {
  ...
  const markedDates = generateMarkedDates(allRecords || []);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {allRecords && allRecords.length > 0 ? allRecords.map((item: any, index: number) => (
          <View key={item.id || index} style={styles.listItem}>
            <View style={styles.listLeft}>
              ...
              <View>
                <Text style={styles.listTitle}>{item.employeeName || 'Employee'}</Text>
                <View style={styles.timeRow}>
                  <Clock color="#F97316" size={14} style={{ marginRight: 4 }} />
                  <Text style={styles.listSubtitle}>{item.date ? item.date.split('T')[0] : 'Unknown Date'} | In: {item.checkIn || '-'} | Out: {item.checkOut || '-'}</Text>
                </View>
              </View>
            </View>
            <View style={[
